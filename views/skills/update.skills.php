    ...
    <main class="mt-14 p-12 ml-0 smXl:ml-64 dark:border-gray-700">
        <form id="addskillform" method="POST" class="max-w-xl mx-auto bg-white border p-8 rounded-2xl ">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" value="<?php echo $result['name'] ?>" id="name" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" placeholder="Jhon doe" required>
                <span class="mt-4 block ml-4 text-xs text-red-600 dark:text-red-400 "></span>
            </div>

            <Button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update Skill</Button>
    ...
    <script>
        ...
        addskillform.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                const response = await fetch("http://peoplepertask_backend_auth.test/skills.php?action=updateskillscript", {
                    method: "POST",
                    body: formData,
                });
